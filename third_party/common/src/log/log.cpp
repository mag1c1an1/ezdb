...
int Log::rename_old_logs() {
  ...
  while (log_index > 0) {
    ...
    std::string log_name_new =
        log_name_ + "." + size_to_pad_str(log_index + 1, 3);
...
const char *lbt() {
  ...
  for (int i = 0; i < size && offset < bt_buffer_size - 1; i++) {
    ...
    offset +=
        snprintf(backtrace_buffer + offset, sizeof(backtrace_buffer) - offset,
                 format, reinterpret_cast<intptr_t>(buffer[i]));
  }
